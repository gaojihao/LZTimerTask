...
#ifdef DEBUG
                        @throw [NSException exceptionWithName:@"LZTimerTask" reason:exception.reason userInfo:exception.userInfo];
#endif
                    }
                });
                repeatCount++;
...
+ (void)countDownTaskWithTarget:(id)target
    ...
    [LZSystemTimeManager sharedInstance].systemTime = serverTime;
    [[LZSystemTimeManager sharedInstance] addObserverForSystemTime:target onChanged:^(id target, id observer, id value, NSDictionary *change) {
        ...
        if (remainSecond > 0 && mode != CountDownModeFinished) {
            ...
            // 倒计时结束，移除kvo
            [[LZSystemTimeManager sharedInstance] removeObserverForSystemTime:observer];
        }
        if (handler) {
